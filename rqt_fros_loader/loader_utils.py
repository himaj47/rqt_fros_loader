...
CHUNCK_SIZE = 1024
class FrosLoader(Node):
    def __init__(self):
        super().__init__('loader_utils')
        logging.info('loader_utils Node Initialized')

        self.overlays = self.search_system_overlays()
        self.update_progress_bar_ = Signal(int)
    ...
    def handle_load(self, selected_option):
        ...
        for chunck in chuncks:
            ...
            self.update_progress_bar_.emit(val)
    ...
    def search_system_overlays(self):
        ...
        try:
            ...
            if not resources:
                ...
                logging.info("No resources of type '%s' found.", resource_type)
                return found_res
            ...
            logging.error("Error locating resources: %s", e)
